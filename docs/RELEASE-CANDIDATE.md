...
The same verified build now has a second deployment target: a public LinuxServer.io Nginx image in GHCR. The image contains the static application and exact CI-built SQLite catalogue, supports AMD64 and ARM64, and is tagged as both `latest` and `sha-<full-commit>`. Image publication does not recreate an operator's running container; deployment automation must pull and recreate it. The full runtime, public access, update, and rollback contract is documented in [CONTAINER-DEPLOYMENT.md](CONTAINER-DEPLOYMENT.md).

GitHub Actions run [33938976627](https://github.com/link42-au/rule1/actions/runs/33938976627) completed `build-sqlite`, `deploy-pages`, and `publish-container` for commit `bcc98dcef67dc6976ee18ee5bbd2c0258f18ec0b`. The published multi-platform image index was `sha256:bd12c133b3f49b3ff3362dfef94e77ca105ecbec601b5a65e2b99a38928ce396`; its embedded database remained SHA-256 `039f02af124b14e35d1c9987ce64cd6addd11c7822bf29de307b0991bf8bf406`. GitHub reports the package as public, and an anonymous GHCR token request fetched the `latest` OCI index with HTTP 200 and the same digest.

Automatic publication is limited to deployable data, application, dependency, container, build-script, and workflow inputs. Pull requests still run the complete verification and container smoke test, while `workflow_dispatch` can force a release from `main`. Documentation-only commit `a64f710f7539f3374dc342e3547c800446406a97` produced no `Build SQLite` run, confirming that documentation maintenance does not rebuild the database, redeploy Pages, or republish the container.
